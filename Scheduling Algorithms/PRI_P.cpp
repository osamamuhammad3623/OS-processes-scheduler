/******************************************************************************
 *
 * Module: priority - Preemptive Scheduler
 *
 * File Name: PRI_P.cpp
 *
 * Description: Source file for priority - Preemptive Scheduler implementation.
 *
 *******************************************************************************/
 ...
 */
vector<Process_Output> PRI_P_Scheduler::getChart(){
	...
	while(buffer.size() != 0 || ReadyQueue.size() != 0)
	{
		...
		ReadyQueue = sorted(ReadyQueue , PRIORITY);

		/*Store the cpu time in case there is an interrupt*/
		float old_CPU_time = CPU_time;
